		...
		if (status != 200 && status != 486) {
			...
			if (sip_user_status.count <= sip_user_ping_max) {
				...
				if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_PING) == SWITCH_STATUS_SUCCESS) {
					...
					switch_event_add_header(event, SWITCH_STACK_BOTTOM, "ping-time", "%d", ping_time);
					switch_event_add_header(event, SWITCH_STACK_BOTTOM, "ping-count", "%d", sip_user_status.count);
